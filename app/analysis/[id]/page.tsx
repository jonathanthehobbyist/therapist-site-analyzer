    ...
    <div className="space-y-2.5">
      <div className="flex items-center gap-4">
        ...
        {isPublic && (
          <>
            <a
              href={`/share/${analysisId}`}
              target="_blank"
              rel="noopener noreferrer"
              className="text-xs text-brand-sky hover:text-brand-sky-vivid transition-colors underline underline-offset-2"
            >
              Open share link
            </a>
            <button
...
function KeywordsTab({ data, sectionDescs, onSaveDescs }: { data: KeywordData; sectionDescs: Record<string, string>; onSaveDescs: (updates: Record<string, string>) => void }) {
  ...
  return (
    <div className="space-y-3">
      ...
      {(!data.strikingDistanceKeywords || data.strikingDistanceKeywords.length === 0) &&
       (data.organicKeywords.length > 0 || data.siteKeywords.length > 0) && (
